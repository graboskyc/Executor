#!/bin/bash

echo
echo "+================================"
echo "| START: Executor Portal"
echo "+================================"
echo

source backend/.env

datehash=`date | md5sum | cut -d" " -f1`
abbrvhash=${datehash: -8}
echo "Using conn string ${MDBCONNSTR}"

echo 
echo "Building container using tag ${abbrvhash}"
echo
docker build -t graboskyc/executor:latest -t graboskyc/executor:${abbrvhash} .

EXITCODE=$?

if [ $EXITCODE -eq 0 ]
    then

    echo 
    echo "Starting container"
    echo
    docker stop executor
    docker rm executor
    docker run -t -i -d -p 9999:80 --name executor -e "PORTAL=${PORTAL}" -e "MDBCONNSTR=${MDBCONNSTR}" -e "SPLUNKSERVER=${SPLUNKSERVER}" -e "SPLUNKINDEX=${SPLUNKINDEX}" -e "REQUIREDAUTHHEADER=${REQUIREDAUTHHEADER}" -e "REQUIREDAUTHGROUP=${REQUIREDAUTHGROUP}" --restart unless-stopped graboskyc/executor:${abbrvhash}

    echo
    echo "+================================"
    echo "| END:  Executor Portal"
    echo "+================================"
    echo
else
    echo
    echo "+================================"
    echo "| ERROR: Build failed"
    echo "+================================"
    echo
fi
